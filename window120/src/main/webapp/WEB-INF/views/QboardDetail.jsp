<%@page import="kr.smhrd.entity.QboardComment"%>
...
<body>
   ...
   <% 
      ...
      List<QboardComment> qbc_list = (List<QboardComment>)session.getAttribute("qbc_list");
   %>
         <!-- Q19. 게시글 세부내용 조회 기능 -->
   <div class="sunken-panel" id="board" style="height: 100%; width: 100%;">
      <table class="interactive" id="list" style="height: 100%; width: 100%;">
         ...
         <thead style="font-size: 23px;">
            ...
            <td>${qboard.qb_title }</td>
            </tr>
            <tr>
               <th style="width:20%">작성자</th>
               <td>${qboard.user_id }</td>
            </tr>

            <!-- 다운로드 -->
            <tr>
               <th>다운로드</th>
               <td><a href="resources/image/${qboard.qb_file }" download>${qboard.qb_file }</a></td>
            </tr>
            <tr>
               <th colspan="2">내용</th>
            </tr>
   ...
   <%
      ...
      <table class="interactive" style="height: 100%; width: 100%;">
         ...
         <%for(QboardComment qbc : qbc_list) { index++;%>
         <tr>
            <td><%=qbc.getUser_id() %></td>
            <td><%=qbc.getCmt_content() %></td>
         </tr>
         <%} %>
         </tbody>
      </table>
   ...
   <div style="text-align: right; padding: 10px;">
            <c:if test="${loginMember.name eq qboard.user_id}">
                    <button onclick="location.href='goQboardUpdate?qb_idx=${qboard.qb_idx}'" >업데이트</button>
               </c:if>
               <c:if test="${loginMember.name eq qboard.user_id}">
                    <button onclick="location.href='QboardDelete?qb_idx=${qboard.qb_idx}'">삭제</button>
               </c:if>
      <button onclick="location.href='goQboard'">뒤로가기</button>
   </div>
